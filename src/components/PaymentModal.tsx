  ...
  const handleInitiateOrder = async () => {
    ...
    try {
      ...
      if (res.status === 401) {
        ...
        setTimeout(() => {
          router.push(`/auth/login?redirect=${encodeURIComponent(`/books/${book.slug}`)}`);
        }, 1200);
        setLoading(false);
        return;
      }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/80 backdrop-blur-md animate-fade-in font-sans overflow-y-auto">
      <div className="bg-[#0C111D] border border-[#232E44] rounded-3xl max-w-lg w-full p-5 sm:p-7 shadow-[0_25px_80px_rgba(0,0,0,0.95)] text-slate-100 relative overflow-hidden my-auto max-h-[92vh] overflow-y-auto">
        ...
        {!paymentSuccess && (
          <button
            onClick={onClose}
            className="absolute top-4 right-4 p-1.5 rounded-lg text-slate-400 hover:text-slate-200 hover:bg-[#1C2638] transition-all relative z-10 cursor-pointer"
            title="Close"
          >
            <X className="w-5 h-5" />
          </button>
        ...
        {!paymentSuccess ? (
          isPendingApproval ? (
            ...
            <div className="text-center py-3 space-y-5 animate-fade-in">
              ...
              <div className="bg-[#070A12] border border-[#1A2336] rounded-2xl p-4 text-left space-y-2.5 max-w-md mx-auto">
              ...
              {/* Reassurance text */}
              <div className="bg-[#070A12] border border-[#1E293E] rounded-xl p-3 text-[11px] text-slate-400 text-left space-y-1">
            ...
            <>
              <div className="flex items-center gap-3 mb-5 border-b border-[#1E293E] pb-4">
                <div className="w-10 h-10 rounded-full bg-rose-500/20 flex items-center justify-center text-rose-300 border border-rose-500/30 shrink-0">
              ...
              {/* BOOK SUMMARY BANNER */}
              <div className="bg-[#070A12] rounded-2xl p-4 border border-[#1E293E] mb-4">
                <div className="flex items-center gap-3">
                  <img
                    src={book.coverImage}
                    alt={book.title}
                    className="w-14 h-20 object-cover rounded-md border border-white/[0.1] shadow-md shrink-0"
                  />
                  <div className="flex-1 min-w-0">
                    <h4 className="font-serif text-base font-bold text-rose-200 truncate">{book.title}</h4>
              ...
              <div className="bg-[#070A12] rounded-xl p-3.5 border border-[#1E293E] mb-5 space-y-2">
                ...
                {!appliedCoupon ? (
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 bg-[#0E1424] border border-[#232E44] focus:border-rose-500 rounded-lg px-3 py-2 text-xs text-slate-200 uppercase font-mono tracking-wider focus:outline-none"
                    />
                    <button
                      onClick={handleApplyCoupon}
                      disabled={validatingCoupon || !couponInput.trim()}
                      className="px-4 py-2 rounded-lg bg-rose-500/20 text-rose-300 border border-rose-500/40 hover:bg-rose-500/30 text-xs font-bold transition-all disabled:opacity-40 cursor-pointer"
                    >
              ...
              <div className="space-y-3 pt-1">
                <button
                  ...
                  disabled={loading}
                  className="w-full py-3.5 px-6 rounded-xl bg-gradient-to-r from-rose-500 via-rose-600 to-rose-700 hover:brightness-110 text-white font-bold text-sm shadow-xl shadow-rose-500/20 transition-all flex items-center justify-center gap-2 disabled:opacity-50 cursor-pointer"
                >
                  {loading ? (
                    <span className="flex items-center gap-2">
                      <Loader2 className="w-4 h-4 animate-spin" />
                      Generating Real UPI Payment QR...
                    </span>
                  ) : (
                    <>
                ...
                <div className="flex items-center justify-between text-[11px] text-slate-400 pt-1 px-1">
                  <span className="flex items-center gap-1">
                    ...
                    Direct Transfer to Author Bank Account
                  </span>
            ...
            <div className="space-y-5 animate-fade-in">
              ...
              {/* QR CODE & DEEP LINK */}
              <div className="bg-[#070A12] p-4 rounded-xl border border-[#1E293E] text-center space-y-3">
                ...
                <div className="pt-1">
                  <a
                    href={upiDeepLink}
                    className="inline-flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-lg bg-rose-500/20 text-rose-300 border border-rose-500/40 text-xs font-bold hover:bg-rose-500/30 transition-all text-center"
                  >
              ...
              <form onSubmit={handleSubmitPaymentProof} className="space-y-4 text-xs">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-[#070A12] border border-[#283652] focus:border-rose-500 rounded-xl p-3 text-slate-200 font-mono tracking-widest text-center text-sm focus:outline-none"
                  />
                ...
                <div>
                  ...
                  {!receiptImage ? (
                    <label className="flex flex-col items-center justify-center border-2 border-dashed border-[#283652] hover:border-rose-500/60 rounded-xl p-4 cursor-pointer bg-[#070A12] transition-all hover:bg-rose-500/5 group">
                    ...
                    <div className="flex items-center gap-3 bg-[#070A12] border border-emerald-500/40 rounded-xl p-2.5">
                      <img
                        src={receiptImage}
                        alt="Receipt Preview"
                ...
                <div className="flex items-center gap-3 pt-2">
                  <button
                    ...
                    className="w-1/3 py-3 px-3 rounded-xl border border-[#283652] hover:border-rose-500/50 text-slate-300 font-bold text-xs hover:bg-[#1C2638] transition-all text-center cursor-pointer"
                  >
                    BACK
                  </button>
                  ...
                  <button
                    ...
                    className="w-2/3 py-3 px-3 rounded-xl bg-gradient-to-r from-rose-500 via-rose-600 to-rose-700 text-white font-bold text-xs shadow-lg shadow-rose-500/20 hover:brightness-110 transition-all disabled:opacity-40 disabled:cursor-not-allowed text-center flex items-center justify-center cursor-pointer"
                  >
                    {loading ? (
                      <span className="flex items-center gap-2">
          ...
          <div className="text-center py-4 space-y-5 animate-scale-up">
            ...
            <div className="pt-2 space-y-3">
              <button
                ...
                className="w-full py-3.5 px-6 rounded-xl bg-gradient-to-r from-rose-500 via-rose-600 to-rose-700 text-white font-bold text-sm shadow-xl shadow-rose-500/30 hover:scale-[1.02] transition-all flex items-center justify-center gap-2 cursor-pointer"
              >
