...
export default function AdminDashboardClient({
  ...
  const [activeTab, setActiveTab] = useState<'analytics' | 'books' | 'manuscripts' | 'coupons' | 'orders'>('analytics');
  ...
  const handleDeleteBook = async (bookId: string, bookTitle: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMsg(`"${bookTitle}" has been deleted.`);
      } else {
        setMsg('Failed to delete book.');
      }
    } catch (err) {
                        ...
                        <tr key={c.id} className="hover:bg-black/20">
                          ...
                          <td className="p-3">
                            <button
                              ...
                              className="py-1 px-2.5 rounded-lg bg-[#161F31] border border-[#293752] hover:border-rose-500/50 text-rose-300 text-[10px] font-bold flex items-center gap-1"
                            >
